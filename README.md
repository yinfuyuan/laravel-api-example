...
Laravel 是一个非常流行的 PHP 框架，我们可以使用它快速构建一个健壮的 WEB 应用。现在 WEB 应用中多会使用前后端分离架构，所以我们经常会遇到使用 Laravel 搭建 WEB API 项目的场景。
Laravel 官方文档在 WEB 方面非常完善，但在 WEB API 方面，很多地方都只提供了规范，虽然我们可以按照自己的风格去使用它，但依然需要一个可以参考的示例。

本项目是一个使用 Laravel 框架实现的 API 项目，它以实际生产环境的项目做为原型，保留了基础的部分作为参考的示例，其思路主要参考 Laravel 官方文档，部分参考 Java 主流框架（Spring）。
项目从零开始，在重要修改都会单独提交 commit 并在末尾添加 [[commit]](https://github.com/yinfuyuan/laravel-api-example/commits/main) 链接, 可自行查看变更内容。

需要注意的是：

1. 这并不是一个 Laravel 的新手教程，文中很多地方需要你了解 Laravel 的基础知识。 [Laravel 官网文档](https://laravel.com/docs)
2. Laravel 各个版本之前实现方式还是有一些区别的（比如异常类中的处理），要注意区分，但整体思路是一样的。

## 安装

Laravel 的安装方式有很多，这里使用 composer 进行安装（不指定安装版本），下面命令会安装最新版本的 Laravel，此项目创建时的 Laravel 版本为 v8.21.0。

    composer create-project laravel/laravel laravel-api-example --prefer-dist

安装好之后需要自行配置项目使其可以对外访问（这里教程非常多），当在浏览器中输入项目地址进入到 Laravel 的欢迎页时，就可以继续向下阅读了。

## 路由

在欢迎页我们可以看到，Laravel 返回的信息是一个 web 页面，也就是 html 代码。这个默认的路由是在 **routes/web.php** 文件中定义的，我们不要在这个文件中定义任何 WEB API 路由，所以需要把这个路由给移除掉。[[commit]](https://github.com/yinfuyuan/laravel-api-example/commit/fa2dbd6b844dc3cf1b4805818c754c308f2135b3)

在 WEB API 项目中，默认所有的路由都需要定义在 **routes/api.php** 文件中，但如果你的路由特别多的话，你也可以在 routes 中定义其他 WEB API 的路由文件，然后在 [RouteServiceProvider](https://github.com/yinfuyuan/laravel-api-example/blob/main/app/Providers/RouteServiceProvider.php) 中按照同样的方式去加载它们。
我们再来看 **RouteServiceProvider** 这个文件，这里定义了路由加载的一些初始值，其中对 **routes/api.php** 文件的加载默认加上 api 前缀，这对 WEB 和 WEB API 混写在同一个项目中很有必要，但单独的 WEB API 项目一般也会使用单独域名。如：

    https://api.apihubs.cn/holiday/get  （这个项目就是使用了这个架构，项目地址：http://www.apihubs.cn/）

所以我们就不需要这个 api 前缀了，这里也可以换成其他前缀，如接口版本号 V1，我们这里选择移除 api 前缀 [[commit]](https://github.com/yinfuyuan/laravel-api-example/commit/df75964574f9a4e3e36044fbdccbb8fafa1bb38e)

在路由文件 **routes/api.php** 中默认的路由是一个需要身份验证的 **/user** 接口，我们以这个接口为例来调整响应信息。

我们先使用浏览器或 postman 访问这个路由的时候，会得到一个错误页面，其中的主要信息为：Route [login] not defined.

我们再使用 ajax 或者在 postman 的 header 中添加 X-Requested-With:XMLHttpRequest 头信息（模拟 ajax）后访问这个路由又会得到一个 JSON 的错误信息：{"message": "Unauthenticated."}

这实际上都是未登录的原因，在未登录访问需要鉴权的接口时 Laravel 会抛出一个 **AuthenticationException** ，而在响应类 **Response** 中会根据请求的 header 头自动做出响应。
也就是如果是以页面的形式访问路由，就会跳转到登录页面，因为项目中还没定义登录页面的路由就出现我们上面看到的那个错误。但如果我们是以接口的形式访问路由就会返回 HTTP 状态码为 401 的 JSON 信息。

但我们这是一个 API 项目，提供出去的都是接口地址，在接口地址中一会返回 JSON 一会又返回一个页面这是不是显得很尴尬，所以我们要把项目改成一个始终以 JSON 格式响应的项目。

这里我们需要新增一个 [Middleware](https://github.com/yinfuyuan/laravel-api-example/blob/main/app/Http/Middleware/JsonApplication.php) （命令如下）来解决这个问题。然后在 [Kernel](https://github.com/yinfuyuan/laravel-api-example/blob/main/app/Http/Kernel.php) 中注册这个 Middleware 使其全局生效。 [[commit]](https://github.com/yinfuyuan/laravel-api-example/commit/3423b75e6cd88608415b55e30da9fa98106ed796)

    php artisan make:middleware JsonApplication

这样我们已经配置好一个 JSON 应用了，在 Laravel 抛出任何异常时，无论我们以什么方式访问都会始终得到 JSON 格式的响应信息。

需要注意的是我们依然不能在路由的闭包或控制器中使用 ~~return view($view)~~ ，因为这会强制返回 html 代码，而 html 代码一定会被浏览器自动解析成页面。
正确的做法是在路由的闭包或控制器中始终 return 一个对象或数组，这两种格式会使 Laravel 自动为我们返回正确的 JSON 信息。
...
在这里我们需要引入一个第三方的库 [phpenum](https://github.com/yinfuyuan/php-enum) （命令如下）。 [[commit]](https://github.com/yinfuyuan/laravel-api-example/commit/34b22459e50a0e93203cd7b647535314ead58914)

    composer require phpenum/phpenum

这是一个枚举库，枚举的功能在这里不过多介绍，在这里我们使用它来定义和管理错误码和错误信息，一般错误码的位数应该是固定的，至少一个模块下的错误码位数是固定的，这里使用 5 位错误码，你可以根据实际使用场景来定义。
我们在 app 目录下新建一个 **Enums** 目录专门管理枚举类，然后添加错误码的枚举类 [ErrorEnum](https://github.com/yinfuyuan/laravel-api-example/blob/main/app/Enums/ErrorEnum.php) （内容如下）为不同的错误和异常定义不同的错误码。 [[commit]](https://github.com/yinfuyuan/laravel-api-example/commit/a79b90a1760780d0dedc6513c69b12b775c148de)

    namespace App\Enums;

    class ErrorEnum extends \PhpEnum\Enum
    {
...
定义好错误码后，我们还需要借助 Laravel 的 [渲染异常](https://laravel.com/docs/8.x/errors#rendering-exceptions) 来渲染自定义异常类 [ApiException](https://github.com/yinfuyuan/laravel-api-example/blob/main/app/Exceptions/ApiException.php) （命令如下）。 [[commit]](https://github.com/yinfuyuan/laravel-api-example/commit/f99e7628f74a156db6c06a1475d343148a567efb)
...
由于接口不再返回任何错误信息了，我们排查问题的方式也只能通过日志来排查 [默认的日志在你本地的这个目录下](https://github.com/yinfuyuan/laravel-api-example/tree/main/storage/logs) Laravel 的日志也是非常强大，你可以随意更改存储的位置和介质，这里就不展开介绍了。
